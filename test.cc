...
extern struct AndList *finalFunction;
...
void q0 (){
	...
	char *cnf = "SELECT o.o_custkey FROM orders AS o, customer AS c, nation AS n WHERE (c.c_custkey = o.o_custkey OR c.c_nationkey = n.n_nationkey)";
	yy_scan_string(cnf);
	yyparse();
	
	double result = s.Estimate(finalFunction, relName, 3);
	printf(" result = %f \n", result);
	if(fabs(result-1500000)>0.1)
		cout<<"error in estimating Q2\n";
	// s.Apply(finalFunction, relName, 3);
